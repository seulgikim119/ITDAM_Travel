...
function SectionHeader({ no, title, desc, icon: Icon }: SectionHeaderProps) {
  return (
    <div className="flex items-start justify-between mb-3">
      <div>
        <p className="text-[#8B95A1]" style={{ fontSize: 11, fontWeight: 700 }}>
          {no}
        </p>
        <h2 className="text-[#191F28] mt-0.5" style={{ fontSize: 19, fontWeight: 800 }}>
          {title}
        </h2>
        <p className="text-[#8B95A1] mt-0.5" style={{ fontSize: 12 }}>
          {desc}
        </p>
      </div>
        ...
        <div className="px-5 pt-1">
          ...
          {preferredTasteTags.length > 0 && (
            <div className="mt-3 rounded-2xl border border-[#F0C070]/35 bg-[#F0C070]/10 px-3 py-2.5">
              <p className="text-[#F0C070]" style={{ fontSize: 11, fontWeight: 800 }}>
                맞춤 취향 반영 완료
              </p>
            ...
            {topStats.map((stat) => (
              ...
              >
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-white/60" style={{ fontSize: 10, fontWeight: 600 }}>
                    {stat.label}
                  </span>
                </div>
                <div className="flex items-end gap-0.5">
                  <span className="text-white" style={{ fontSize: 30, fontWeight: 800 }}>
                    {stat.value}
                  </span>
                  <span className="text-white/50 mb-[2px]" style={{ fontSize: 10, fontWeight: 600 }}>
                    {stat.unit}
                  </span>
                </div>
              </div>
            ...
            {feedRoutines.map((routine) => (
              <article key={routine.id} className="rounded-2xl border border-[#F0E6D0] overflow-hidden">
                <div className="h-[132px] relative">
                  ...
                  <div className="absolute left-3 top-3 px-2.5 py-1 rounded-lg bg-black/45 text-white" style={{ fontSize: 11, fontWeight: 700 }}>
                ...
                <div className="p-3.5">
                  <p className="text-[#4E5968] mb-2" style={{ fontSize: 12 }}>
                    {routine.subtitle}
                  </p>
                  <p className="text-[#191F28] mb-3 truncate" style={{ fontSize: 13, fontWeight: 700 }}>
                    경로: {routine.routeSummary}
                  </p>

                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-2.5 py-1 rounded-lg bg-[#F2F4F6] text-[#4E5968]" style={{ fontSize: 11, fontWeight: 700 }}>
                      {routine.duration}
                    </span>
                    <span className="px-2.5 py-1 rounded-lg bg-[#F2F4F6] text-[#4E5968]" style={{ fontSize: 11, fontWeight: 700 }}>
                      예산 {routine.budget}
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      style={{ fontSize: 13, fontWeight: 700, background: "linear-gradient(135deg, #F0C070, #E8A830)" }}
                    >
              ...
              regionRanking.map((item, idx) => (
                <div key={item.region} className="flex items-center justify-between h-12 px-3 rounded-xl bg-[#FFF9EE]">
                  <div className="flex items-center gap-2">
                    <span className="w-6 h-6 rounded-full bg-[#FDF0D6] text-[#E8A830] flex items-center justify-center" style={{ fontSize: 12, fontWeight: 800 }}>
                      {idx + 1}
                    </span>
                    <p className="text-[#191F28]" style={{ fontSize: 14, fontWeight: 700 }}>{item.region}</p>
                  </div>
                  <p className="text-[#4E5968]" style={{ fontSize: 12, fontWeight: 700 }}>
            ...
            {themeCurations.map((curation) => (
              ...
              >
                <div className="h-[120px] relative">
                  ...
                  <span className="absolute top-3 left-3 px-2.5 py-1 rounded-lg bg-white/90 text-[#E8A830]" style={{ fontSize: 11, fontWeight: 800 }}>
                    {curation.badge}
                  </span>
                </div>
                <div className="p-3">
                  <p className="text-[#191F28]" style={{ fontSize: 15, fontWeight: 800 }}>{curation.title}</p>
                  <p className="text-[#6B7684] mt-1" style={{ fontSize: 12, lineHeight: 1.45 }}>{curation.description}</p>
                  <div className="mt-2 flex items-center text-[#E8A830]" style={{ fontSize: 12, fontWeight: 700 }}>
                    테마 상세 보기
