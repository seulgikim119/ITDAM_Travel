    ...
    <div className="h-full flex flex-col bg-[#FFF8E7]">
      <div className="bg-white rounded-b-[24px] shadow-sm">
        ...
        <div className="flex items-center justify-between px-4 py-2">
          ...
          <h2 className="text-[#2C2C2A]" style={{ fontSize: 17, fontWeight: 600 }}>
            내 루트
          </h2>
          <button className="p-2 text-[#E8A830]" style={{ fontSize: 14, fontWeight: 500 }}>
            수정
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 px-5 pt-5 overflow-y-auto">
        ...
        <div className="relative">
          {spots.map((spot, i) => (
            <div key={spot.name} className="flex gap-4 mb-1">
              <div className="flex flex-col items-center">
                <div className="w-3 h-3 rounded-full mt-1.5" style={{ backgroundColor: spot.color }} />
                {i < spots.length - 1 && (
                  <div
                    className="w-[2px] flex-1 my-1"
                    style={{ background: `linear-gradient(${spot.color}, ${spots[i + 1].color})`, opacity: 0.3 }}
                  />
                )}
              </div>
